...
pub const KNOWLEDGE_MIGRATIONS: &[&str] = &[V1, V2, V3, V4, V5, V6, V7, V8, V9, V10, V11, V12, V13, V14, V15, V16, V17, V18, V19, V20, V21, V22, V23, V24, V25, V26, V27, V28, V29, V30, V31, V32, V33, V34, V35, V36];

/// Table names reserved by samgraha's own migrations. A standard's custom
/// data tables (§2.2 of the custom-workflow-db proposal) must not collide
/// with any name in this list — enforced at `register_standard` time.
pub const RESERVED_TABLE_NAMES: &[&str] = &[
    // V1
    "_schema_version", "documents", "relationships", "audit_results",
    "glossary", "enrichment", "build_metadata",
    // V2
    "search_index",
    // V3
    "knowledge_packages",
    ...
    "semantic_reports",
    // V12
    "section_audit_hashes",
    // V13
    "pipeline_reports", "pipeline_findings", "report_comments",
    // V14
    "build_reports", "security_reports", "consistency_reports",
    "coverage_reports", "report_findings", "report_evidence",
    "report_summaries", "report_improvements", "report_recommendations",
    // V15-V27 (one *_reports table per documentation domain)
    "architecture_reports", "vision_reports", "design_reports",
    "readme_reports", "prototype_reports", "external_context_reports",
    "engineering_reports", "feature_reports", "feature_technical_reports",
    "feature_design_reports", "deterministic_runtime_reports",
    "external_context_ownership_reports", "implementation_reports",
    // V28
    "fix_sessions", "fix_attempts", "fix_plans", "fix_plan_steps",
    // V29
    "project_plans", "project_phases",
    // V30
    "help_reports", "repository_metadata",
    // V31
    "documentation_structure_reports",
    // V32
    "pipeline_semantic_reports", "summary_reports",
    // V33
    "standard_audit_runs",
    // V34
    "custom_data_tables",
...
const V34: &str = "
-- V34: Catalog of custom data tables declared by standards. Each row
--      represents one table a standard's own script creates inside
--      knowledge.db (prefixed per §2.2 of the custom-workflow-db
--      proposal). samgraha never touches these tables' schema — only
--      catalogs them for discoverability and collision detection.
CREATE TABLE IF NOT EXISTS custom_data_tables (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    standard        TEXT    NOT NULL,
    table_name      TEXT    NOT NULL,
    purpose         TEXT    NOT NULL DEFAULT '',
    owner_script    TEXT    NOT NULL DEFAULT '',
    UNIQUE(standard, table_name)
);
CREATE INDEX IF NOT EXISTS idx_custom_data_tables_standard ON custom_data_tables(standard);
";

const V35: &str = "
-- V35: Adds shape_json to custom_data_tables (V34). Populated by
--      PRAGMA table_info introspection after the owner_script has
--      created the actual table — never hand-authored, so it can't
--      drift from what the table really looks like. Stays NULL until
--      the first successful introspection (table may not exist yet
--      at registration time).
ALTER TABLE custom_data_tables ADD COLUMN shape_json TEXT;
";

const V36: &str = "
-- V36: Catalog of every named script/prompt asset a standard ships,
--      independent of which phase(s) reference it. Lets an LLM (or
--      samgraha's dispatcher) resolve a name to a path and read the
--      one-line purpose without inspecting the filesystem.
--      Mirrors schema/knowledge-hub/29-standard_assets.sql (which
--      lives in standards.db); this copy is for knowledge.db so
--      local queries don't cross databases.
...
CREATE INDEX IF NOT EXISTS idx_standard_assets_standard ON standard_assets(standard, kind);
";
