...
pub struct SyncResult {
    ...
    /// Number of seed-once (audit/analysis) files copied. Only counts
    /// files that did not already exist locally — zero on re-sync
    /// when all files were already present.
    #[serde(default)]
    pub seed_once_synced: usize,
}
...
pub fn sync_knowledge_system(root: &Path) -> Result<SyncResult> {
    ...
    // 3b. Seed-once sync for audit/analysis assets (§7).
    //     On first init: copy from global store to local .samgraha/<dir>/.
    //     On re-sync: never overwrite existing local files — only copy
    //     files that are missing locally.
    //     Reads directory names from system-assets.json if available,
    //     falls back to legacy "audit_analysis" single dir.
    let mut seed_once_synced = 0usize;
    let aa_dirs: Vec<String> = {
        let assets_path = sys_dir.join("system-assets.json");
        if assets_path.exists() {
            if let Ok(content) = std::fs::read_to_string(&assets_path) {
                if let Ok(val) = serde_json::from_str::<serde_json::Value>(&content) {
                    if let Some(arr) = val.get("audit_analysis").and_then(|v| v.as_array()) {
                        arr.iter()
                            .filter_map(|v| v.as_str().map(|s| s.to_string()))
                            .collect()
                    } else {
                        vec!["audit_analysis".to_string()]
                    }
                } else {
                    vec!["audit_analysis".to_string()]
                }
    ...
    for aa_dir_name in &aa_dirs {
        let source_aa = sys_dir.join(aa_dir_name);
        if source_aa.is_dir() {
            let local_aa = root.join(".samgraha").join(aa_dir_name);
            if !local_aa.exists() {
                // First init — full copy.
                std::fs::create_dir_all(&local_aa)?;
                seed_once_synced += common::fs_sync::copy_dir_recursive(
                    &source_aa, &local_aa, &[],
                )?;
            } else {
                // Re-sync — seed-once: only copy missing files.
                for entry in walkdir::WalkDir::new(&source_aa)
                    .into_iter()
                    .filter_map(|e| e.ok())
                    .filter(|e| e.file_type().is_file())
                {
                    let rel = entry.path().strip_prefix(&source_aa)
                        .map_err(|e| anyhow::anyhow!("Strip prefix failed: {}", e))?;
                    let dst = local_aa.join(rel);
                    if !dst.exists() {
