...
/// hardcoded domain list. No file is read here — `ctx.standard` is already
/// fully populated from `StandardRegistry` by `ProjectContext::detect_with_registry`.
///
/// All `ProjectCase` variants route here. The legacy hardcoded planners
/// (NewProjectPlanner, DocAuditPlanner, ImplTestAuditPlanner,
/// BuildAuditPlanner) have been removed — their domain lists are now
/// defined by the standard's own `domains_by_tier` + `plan_scenarios`.
...
/// All cases route to `StandardWorkflowPlanner` — the legacy hardcoded
/// planners have been removed; domain lists come from the standard's
/// `domains_by_tier` + `plan_scenarios`.
pub fn resolve_planner(case: &ProjectCase) -> Box<dyn ProjectPlanner> {
    match case {
        ProjectCase::NewProject => Box::new(StandardWorkflowPlanner),
        ProjectCase::DocAudit => Box::new(StandardWorkflowPlanner),
        ProjectCase::ImplTestAudit => Box::new(StandardWorkflowPlanner),
        ProjectCase::BuildAudit => Box::new(StandardWorkflowPlanner),
...
mod tests {
    ...
    #[test]
    fn test_resolve_planner_routes_all_cases_to_standard() {
        for case in [ProjectCase::NewProject, ProjectCase::DocAudit, ProjectCase::ImplTestAudit, ProjectCase::BuildAudit, ProjectCase::Standard] {
            let planner = resolve_planner(&case);
            assert_eq!(planner.case(), ProjectCase::Standard);
        }
    }

    #[test]
    fn test_phase_dependencies_are_sequential() {
        // StandardWorkflowPlanner with standard context produces sequential deps.
        let scenarios = vec![
            scenario("new", "no_documentation", 1, "generation"),
            scenario("new", "no_documentation", 1, "audit"),
            scenario("new", "no_documentation", 1, "fix"),
        ...
        domains_by_tier.insert(1, vec!["infrastructure".to_string()]);
        let ctx = standard_ctx("new", false, scenarios, domains_by_tier);
        let result = StandardWorkflowPlanner.generate_plan(&ctx, "test-dep");
