...
| 7 | Genericity proof: sketch (not necessarily fully author) a second `system` — enough of a hackathon-eval or research-paper-publishing standard to confirm Phase 1's schema needs zero changes to hold it. This is the actual test of §4a's central claim; do it once Phase 3 is stable, not before, so the proof is against a real second workload rather than a hypothetical one. | A few rows/files for one pilot second system | 3 |
